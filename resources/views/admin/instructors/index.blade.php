...
<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-6 text-gray-900">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('admin.instructors.create') }}" class="bg-gray-800 hover:bg-gray-900 text-white font-bold py-2 px-4 rounded inline-flex items-center">
                    ...
                    @foreach($instructors as $instructor)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-sm text-gray-500">{{ $instructor->phone }}</div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <form action="{{ route('admin.instructors.toggle-active', $instructor) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $instructor->active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                        {{ $instructor->active ? 'Active' : 'Inactive' }}
                                    </button>
                                </form>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('admin.instructors.edit', $instructor) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
